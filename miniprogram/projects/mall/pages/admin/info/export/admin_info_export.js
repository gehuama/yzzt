...
Page({
	...
	_loadDetail: async function (isDel) {
		...
		let data = await cloudHelper.callCloudData('admin/info_data_get', params, options);
	...
	bindOpenTap: function (e) {
		fileHelper.openDoc('投诉与建议数据', this.data.url);
	},
	...
	bindExportTap: async function (e) {
		try {
			...
			await cloudHelper.callCloudData('admin/info_data_export', params, options).then(res => {
	...
	bindDelTap: async function (e) {
		try {
			...
			await cloudHelper.callCloudData('admin/info_data_del', {}, options).then(res => {
				this.setData({
					url: '',
					time: ''
				});
